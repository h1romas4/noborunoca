...
extern uint8_t music_title[], music_main[], music_game_over[],
    music_game_over_2[], sound_extend[];
// 特殊効果音（ジャンプ）
...
void init_graphics() {
    ...
    *(uint8_t *)MSX_CLIKSW = 0;
    ...
    // PCG 設定(3面に同じデーターを転送)
    vwrite((char *)pcg_char, 0x0000, 0x800);
    vwrite((char *)pcg_char, 0x0800, 0x800);
    vwrite((char *)pcg_char, 0x1000, 0x800);

    // 色設定(3面に同じデーターを転送)
    vwrite((char *)pcg_color, 0x2000, 0x800);
    vwrite((char *)pcg_color, 0x2800, 0x800);
    vwrite((char *)pcg_color, 0x3000, 0x800);
...
void print_scaffold_line(uint8_t y, uint32_t scaffold, uint8_t char_code) {
    ...
    // ブロック転送
    vwrite((char *)x_vram, VPOS(1, y), STAGE_WIDTH - 2);
}
...
void print_scaffold(uint8_t range_y) {
    for (uint8_t y = 0; y <= range_y; y++) {
        ...
        if (y == 0)
            continue;
        // 仮想足場上、足場がなければ処理しない
        if ((game_stage.scaffold[y] & GAME_SCAFFOLD_FILL) == 0)
            continue;
        // 足場を VRAM に書き込み
...
void scroll_scaffold_line(void) {
    ...
    for (uint8_t y = 0; y < STAGE_HEIGHT; y++) {
        ...
        // メインメモリーから VRAM に転送
        vwrite((char *)x_vram_trance_upper, addr, STAGE_WIDTH - 2);
        if (y < STAGE_HEIGHT - 1) {
            vwrite((char *)x_vram_trance_lower, addr + VRAM_WIDTH,
                   STAGE_WIDTH - 2);
        }
    }
}
...
void scroll_scaffold() {
    ...
    if (game_stage.scroll_adjust > 8) {
        ...
        // 仮想 VRAM をスクロール
        memmove(&game_stage.scaffold[1], &game_stage.scaffold[0],
                sizeof(uint32_t) * (STAGE_HEIGHT));
        // スクロール外の足場生成
        create_scaffold(0);
    }
}
...
void calc_power() {
    ...
    // power_v を考慮してパワーゲージに 0(GAME_POWER_MIN) - 56(GAME_POWER_MAX)
    // でキャップをかける
...
void sprite_my() {
    ...
    if (my.y < 0) {
        if (my.y >= -16) {
            ...
            put_sprite_16(my.sprite_no + 1, 0, HIDDEN_SPRITE_Y,
                          my.sprite_no + 1, 0xc);
            return;
        }
    }
    ...
    if (my.sprite_no != sprite_no) {
        put_sprite_16(my.sprite_no, 0, HIDDEN_SPRITE_Y, my.sprite_no, 1);
        put_sprite_16(my.sprite_no + 1, 0, HIDDEN_SPRITE_Y, my.sprite_no + 1,
                      0xc);
    }
...
void move_my(uint8_t jump_power) {
    ...
    } else if (my.can_jump && adjust_jump_power == 0 &&
               game_stage.scroll_adjust > 0) {
        // 歩き中は強制スクロール分調整
        my.y = vram_y * 8 - 16 + scroll_adjust;
    }
...
void game_main(uint8_t trigger) {
    ...
    if (game.level < 10 && game.tick++ % 3 == 0)
        return;
...
void init_sound_driver() {
    ...
    uint16_t hook = (uint16_t)sounddrv_exec; // NOLINT
    h_time[0] = 0xc3;                        // JP
